...
mkdir -p volumes-dev/{ssb-data,ai-models,logs} volumes-dev/client-state/banking
...
chmod 755 volumes-dev/ai-models volumes-dev/client-state volumes-dev/client-state/banking 2>/dev/null || true
...
echo "- logs/        <- /app/logs"
echo "- client-state/ <- /app/state: oasis-config.json de la GUI + banking/ (mapa de direcciones ECOin); sobrevive a recreate/rebuild"
