...
Si despliegas en ese host, **no** empieces desde `.env.example`. Usa `pub/.env.vps.example`, porque el perfil genérico sigue pensado para rutas relativas cómodas en local.
...
- `oasis-hub` (**activo desde 2026-09-13**, WP-O46): HUB clearnet `/c` como **nodo de soporte**: misma imagen,
  `command: ["backend"]`, identidad SSB propia, estado en `/srv/oasis/oasis-hub`. Solo red Docker.
- `hub-cache` (**activo desde 2026-09-13**, WP-O46): nginx con caché en disco acotada delante de `oasis-hub`.
...
1. Prepara la base del host con `devops/scripts/bootstrap-debian13-base.sh`.
...
No hace falta parar `oasis-client` (cliente en `:3000`): la UI de mantenimiento usa **`3001` por defecto**. Sí hay que parar el **pub SSB** (`--stop-pub`, incluido en `maint-ui:up`).

**Identidad:** este flujo edita el pub **local** (`volumes-dev/oasis-pub/ssb-data`), no `pub.escrivivir.co` en el VPS. Para el pub de producción (`@/snvahva…`) usa `devops/scripts/pub-maint-ui.sh` + túnel SSH (README DevOps).
...
En producción (VPS): `bash devops/scripts/pub-maint-ui.sh up --stop-pub` + túnel SSH (ver README DevOps).
...
La primera vez se crea `pub/.env.local` automáticamente a partir de `.env.local.example`.
...
4. Abre `pub/pub.code-workspace`.
5. En el VPS, crea `pub/.env` a partir de `pub/.env.vps.example`.
...
Compilar y copiar el resultado a `pub/site/`, dejando Caddy como servidor estático.

### Angular SSR / Node

Añadir el proyecto en `pub/frontend` o configurar `PUB_FRONTEND_DIR` en `.env`, asegurar que expone `8080`, y levantar el profile `frontend`.
