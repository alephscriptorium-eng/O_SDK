...
if [ -z "$INVITE_CODE" ]; then
  echo "Usage: INVITE='host:port:@key~secret' bash pub/scripts/join-prod-client.sh"
  echo "   or: bash pub/scripts/join-prod-client.sh 'host:port:@key~secret' [client_url]"
  exit 2
fi
