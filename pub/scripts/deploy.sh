...
DEPLOY_LOG="$PUB_DIR/../devops/scripts/deploy-log.sh"
if [ -f "$DEPLOY_LOG" ]; then
  ...
  _feed="$(compose_pub exec -T oasis-pub sh -lc 'node /app/pub/tools/ssb-admin.js whoami 2>/dev/null' 2>/dev/null | grep -oE '@[A-Za-z0-9+/]+=\.ed25519' | head -1 || true)"
