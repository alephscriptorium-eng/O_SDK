...
async function getPubLiveInfo() {
  ...
  const whoamiOut = await dockerExec(pubContainer, 'node /app/pub/tools/ssb-admin.js whoami 2>/dev/null').catch(() => '');
...
async function getInvite() {
  ...
  const out = await dockerExec(pubContainer, `node /app/pub/tools/ssb-admin.js invite.create ${inviteUses} 2>/dev/null`).catch(() => '');
