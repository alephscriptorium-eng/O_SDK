...
- El pub es un servicio dedicado: no se mezcla con el cliente `oasis-client` actual.
...
Todo el material de despliegue (claves SSH, scripts de bootstrap, runbooks) vive en `devops/`, fuera de `pub/` y con `.gitignore` deny-by-default. La clave SSH `gandi_pub_ed25519` se genera con `devops/scripts/init-ssh-key.sh`.
...
- [x] Crear clave SSH local dedicada para el VPS (`devops/.ssh/gandi_pub_ed25519`).
...
- Evidencia operativa consolidada en `devops/SERVER.md`.
...
- Perfil de despliegue real preparado como `pub/.env.prod`, derivado de `.env.vps.example`, con bind mounts absolutos a `/srv/oasis/oasis-pub/*`.
...
- Al intentar aceptar el invite desde el cliente Oasis local en `http://localhost:3000/invites`, el contenedor `oasis-client` registra errores SHS como:
...
1. **Cliente local desactualizado o imagen stale**: el contenedor `oasis-client` puede no estar ejecutando el `src/server` actual que carga `ssb-invite-client`, `ssb-ebt` y `caps.shs` Oasis.
...
- [ ] Comprobar dentro de `oasis-client` el runtime real:
  - `caps.shs`
  - `config.path`
...
- [ ] Si el fallo es de imagen stale, reconstruir/recrear solo `oasis-client` con la codebase actual sin borrar el volumen SSB local salvo backup explícito.
...
- Se documenta en `devops/SERVER.md` la evidencia final sin guardar invites vivos en el repo.
...
- El contenido servido en `/` se decide por:
  - `OASIS_PUB_FRONT_MODE=static` → bind mount de `pub/site/` (lo de hoy).
...
- [ ] Si es estática: compilar a `pub/site-angular/` y montar en Caddy.
...
- **A)** Servirla desde el mismo `pub-web` en una ruta secundaria, p. ej. `/scriptorium/`, con bind mount de `pub/site/scriptorium/`.
...
- [ ] Crear esqueleto en `pub/site/scriptorium/index.html` con look & feel poster-template.
