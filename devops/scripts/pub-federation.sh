...
# Datos de instancia (IP, clave, cap...) → devops/hosts/<DEVOPS_HOST>/host.env
...
# Se define por instancia en host.env (EXPECTED_SHS).
EXPECTED_SHS="${EXPECTED_SHS:-}"

# Seed feed to federate with (per-instance, host.env SNH_FEED).
SNH_FEED="${SNH_FEED:-}"
...
Examples:
  ...
  bash devops/scripts/pub-federation.sh announce --yes
  bash devops/scripts/pub-federation.sh follow-solarnethub --dry-run
  bash devops/scripts/pub-federation.sh follow-solarnethub --yes
